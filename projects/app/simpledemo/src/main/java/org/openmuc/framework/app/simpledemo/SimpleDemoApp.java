        ...
        // try {
            ...
            for(int i = 0; i < 2; i++){
                for(int j = 0; j < 100; j++){
                    ...
                    Value temperature = dataAccessService.getChannel("str" +(i+1)+ "_cell"+ (j+1)+"_V").getLatestRecord().getValue();
                    Value resistance = dataAccessService.getChannel("str" +(i+1)+ "_cell"+ (j+1)+"_V").getLatestRecord().getValue();
